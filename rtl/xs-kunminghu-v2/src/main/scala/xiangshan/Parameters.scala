...
(
  ...
  */
  ...
  val vfSchdParams = {
    ...
    SchdBlockParams(Seq(
      IssueBlockParams(Seq(
        // VFEX0 also hosts the custom vdot.vv unit; it reuses the existing
        // VfRD(0..2)/V0RD/VlRD read ports and the VfWB(0,0)/V0WB(0,0) write ports
        // (VdotCfg has the same 5-source layout as VimacCfg), so no new VPRF ports.
        ExeUnitParams("VFEX0", Seq(VfmaCfg, VialuCfg, VimacCfg, VdotCfg, VppuCfg), Seq(VfWB(port = 0, 0), V0WB(port = 0, 0)), Seq(Seq(VfRD(0, 0)), Seq(VfRD(1, 0)), Seq(VfRD(2, 0)), Seq(V0RD(0, 0)), Seq(VlRD(0, 0)))),
