...
class VecExceptionGen(implicit p: Parameters) extends XSModule{
  ...
  // Custom vdot.vv: fixed INT8 x INT8 -> INT32 semantics, one uop per lmul
  // group (VEC_VVV). It does NOT use the RVV 2-uop widening lists above;
  // instead it requires vsew = e32 so that vl (in e32 units, VLMAX = 4 per
  // 128-bit register for m1) matches the 4 x INT32 dest elements per uop.
  private val vdotInst = VDOT_VV === inst.ALL
  private val vdotEewIllegal = vdotInst && SEW =/= VSew.e32

  private val eewIllegal = fpEewIllegal || intExtEewIllegal || wnEewIllegal || vdotEewIllegal
