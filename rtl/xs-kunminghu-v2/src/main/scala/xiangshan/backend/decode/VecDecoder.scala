...
import xiangshan._
import yunsuan.{VfpuType, VipuType, VimacType, VpermType, VialuFixType, VfaluType, VfmaType, VfdivType, VfcvtType, VidivType, VdotType}
import xiangshan.backend.decode.Zvbb._
import xiangshan.backend.decode.Vdot._
...
object VecDecoder extends DecodeConstants {
  ...
  val opmvv: Array[(BitPat, XSDecodeBase)] = Array(
    ...
    // custom vdot.vv: INT8 x INT8 -> INT32 dot product, accumulate into vd.
    // One uop per lmul group (each uop produces a full 128-bit vd fragment),
    // so it reuses the plain VEC_VVV split instead of the VVW widening split.
  ...
  val opfff: Array[(BitPat, XSDecodeBase)] = Array(
    ...
    FNMADD_S -> OPFFF(SrcType.fp, SrcType.fp, SrcType.fp, FuType.fmac, VfmaType.vfnmacc, F, T, F, UopSplitType.SCA_SIM),
    FNMSUB_S -> OPFFF(SrcType.fp, SrcType.fp, SrcType.fp, FuType.fmac, VfmaType.vfnmsac, F, T, F, UopSplitType.SCA_SIM),
    FMADD_D  -> OPFFF(SrcType.fp, SrcType.fp, SrcType.fp, FuType.fmac, VfmaType.vfmacc , F, T, F, UopSplitType.SCA_SIM),
