...
case class FuConfig (
  ...
  def needVecCtrl: Boolean = {
    import FuType._
    Seq(vipu, vialuF, vimac, vidiv, vfpu, vppu, vfalu, vfma, vfdiv, vfcvt, vldu, vstu, vdot).contains(fuType)
  }
  ...
  def isVecArith: Boolean = fuType == FuType.vialuF || fuType == FuType.vimac ||
                            ...
                            fuType == FuType.vidiv || fuType == FuType.vdot
...
object FuConfig {
  ...
  // Custom vdot.vv (INT8 x INT8 -> INT32 dot product, accumulate).
  // Follows the VimacCfg template: same 5-source layout (vs1/vs2/oldVd/v0/vl),
  // pipelined with 2-cycle latency matching the two register stages inside Vdot64b.
  // No vxsat (wrap-around accumulation, no saturation), so writeVxsat stays false.
  val VdotCfg = FuConfig (
    name = "vdot",
    fuType = FuType.vdot,
    fuGen = (p: Parameters, cfg: FuConfig) => Module(new VdotU(cfg)(p).suggestName("Vdot")),
    srcData = Seq(
      Seq(VecData(), VecData(), VecData(), V0Data(), VlData()), // vs1, vs2, vd_old, v0, vtype&vl
    ),
    piped = true,
  ...
  def VecArithFuConfigs = Seq(
    VialuCfg, VimacCfg, VdotCfg, VppuCfg, VipuCfg, VfaluCfg, VfmaCfg, VfcvtCfg
  )
}
