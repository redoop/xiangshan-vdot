...
object FuType extends OHEnumeration {
  ...
  // custom vector dot product (vdot.vv, INT8 x INT8 -> INT32 accumulate)
  // NOTE: appended at the end on purpose so all pre-existing one-hot IDs stay stable
  val vdot = addType(name = "vdot")
  ...
  val vecOPI = Seq(vipu, vialuF, vppu, vimac, vidiv, vdot)
